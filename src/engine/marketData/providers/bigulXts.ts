...
export class BigulXtsProvider implements MarketDataProvider {
 ...
 async getDailyCandles(ticker:string,exchange:'NSE'|'BSE'){const x=await this.request('/marketdata/candles?exchange='+encodeURIComponent(exchange)+'&symbol='+encodeURIComponent(ticker)+'&interval=1d');return x.data as any;}
