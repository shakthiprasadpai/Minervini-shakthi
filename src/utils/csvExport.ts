...
export const exportPortfolioToCsv = (holdings: PortfolioHolding[]) => {
  ...
  const rows = holdings.map((h) => {
    ...
    return [
      ...
      escapeCsvCell(h.entryPrice.toFixed(2)),
      escapeCsvCell(h.currentPrice.toFixed(2)),
      ...
      escapeCsvCell(h.stopLossPrice.toFixed(2)),
      escapeCsvCell(riskDollar > 0 ? riskDollar.toFixed(2) : '0.00'),
      escapeCsvCell(h.pivotTargetPrice.toFixed(2)),
...
export const exportTradePlansToCsv = (stocks: MinerviniTradeSetup[]) => {
  ...
  const rows = stocks.map((s) => {
    ...
    return [
      ...
      escapeCsvCell(s.currentPrice.toFixed(2)),
      escapeCsvCell(s.pivotPrice.toFixed(2)),
      escapeCsvCell(s.stopLossPrice.toFixed(2)),
      escapeCsvCell(`-${s.stopLossPercent}%`),
      escapeCsvCell(s.target1Price.toFixed(2)),
      escapeCsvCell(`+${s.target1Percent.toFixed(1)}%`),
      escapeCsvCell(`${s.riskRewardRatio.toFixed(2)}:1`),
      escapeCsvCell(`${s.volumeDryUpPercent}%`),
      escapeCsvCell(s.pivotVolume ? Math.round(s.pivotVolume) : 'N/A'),
      escapeCsvCell(s.sma50.toFixed(2)),
      escapeCsvCell(s.sma150.toFixed(2)),
      escapeCsvCell(s.sma200.toFixed(2)),
