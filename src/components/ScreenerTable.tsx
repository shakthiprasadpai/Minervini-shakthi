...
export const ScreenerTable: React.FC<ScreenerTableProps> = ({
  ...
  const getHoldingDays = (stock: MinerviniTradeSetup): { min: number; max: number } => {
    switch (stock.vcpStage) {
      case 'Active Breakout': return { min: 10, max: 30 };
      case 'Breakout Pending': return { min: 5, max: 15 };
      case 'T4': return { min: 10, max: 25 };
      case 'T3': return { min: 15, max: 30 };
      case 'T2':
      default: return { min: 20, max: 40 };
    }
  };
          ...
          <table className="w-full min-w-[2000px] text-left border-collapse">
            <thead>
              <tr className="border-b border-[#e5e4e1] text-[10px] uppercase tracking-[0.2em] text-[#b5a68d] font-bold bg-[#f9f8f5]">
                ...
                <th className="py-3 px-2.5 text-center">Entry</th>
